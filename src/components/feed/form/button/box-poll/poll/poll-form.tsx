...
const AddPollDurationForm = dynamic(() => import('./add-poll-duration'));
...
function PollForm({ feed, onChange }: P) {
  ...
  const [expiredPollAt, setExpiredPollAt] = useState<any>(
    moment().endOf('day').add(7, 'days').toDate()
  );

  const onChangePollDuration = (numberDays) => {
    const date = !numberDays
      ? moment().endOf('day').add(99, 'years').toDate()
      : moment().endOf('day').add(numberDays, 'days').toDate();
    ...
    <>
      <div className={classNames(style['poll-form'])}>
          ...
          {!feed ? (
            <span aria-hidden onClick={() => setOpenPollDuration(true)}>
              ...
              {!expirePollDays
                ...
                : `${expirePollDays} ${intl.formatMessage({
                  id: 'days',
                  defaultMessage: 'days'
                })}`}
            </span>
        ...
        <Form.Item
          ...
          rules={[
            {
              required: pollList.length > 0,
              message: intl.formatMessage({
                id: 'pleaseAddAQuestion',
        ...
        {!feed && pollList.length > 1 && (
          <div className={style['poll-form-footer']}>
            <a aria-hidden onClick={() => setPollList(pollList.concat(['']))}>
              {intl.formatMessage({
                id: 'addAnotherOption',
